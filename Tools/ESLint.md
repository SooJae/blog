...
Linter를 사용하는 이유도 결국은 유지보수를 위함입니다.
특히 느슨한 형식의 언어인 Javascript에서는 코드 오류가 자주 발생합니다.
하지만 JavaScript는 동적 분석을 하기 때문에 이러한 오류를 찾기위해서는 코드를 직접 실행해서 확인을 해봐야 합니다.
...
eslint 라이브러리를 설치한 후에 초기화를 해줍시다.
...
이후 .eslintrc.js 파일이 생성되며 ESLint에 대한 설정을 추가적으로 할수 있습니다.
package.json 파일 내의eslintConfig 부분에서도 설정할 수 있습니다. 

.eslintignore 파일을 생성하여 해당 파일, 폴더들에 대한 ESLint를 무시할 수도 있습니다.(/node_modules/* and /bower_components/* in the project root는 기본적으로 제외되어 있습니다.)
...
매번 eslint를 실행하는 것 조차 번거로울 뿐만아니라, 같은 팀 내에서 실수로 eslint를 실행하지 않고 저장소에 푸시를 할 수도 있습니다.
그렇게 되면 깃 로그에 eslint 적용 같은 쓸데 없는 로그들이 쌓이게 됩니다.
이제 그런 가능성 조차 없애버립시다. 
깃 푸시 전에 코드를 ESLint로 강제해봅시다.

다음 라이브러리들이 추가로 필요합니다.
